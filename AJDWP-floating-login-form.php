...
function pl_custom_user_login() {

	// Verify the nonce
	if (!isset($_POST['pl_user_quick_login_field']) || !wp_verify_nonce($_POST['pl_user_quick_login_field'], 'pl_custom_user_login_nonce')) {
...
add_action('wp_ajax_pl_custom_user_login', 'pl_custom_user_login');
add_action('wp_ajax_nopriv_pl_custom_user_login', 'pl_custom_user_login'); // For non-logged in users


function pl_quick_login_right_bottom_func(){
...
?>
	...
	<div class="form-popup" id="myForm">
		<div class="form-container" id="form-container">
			...
			<?php 
				wp_nonce_field('pl_custom_user_login_nonce', 'pl_user_quick_login_field');
	...
	<script>
		...
		jQuery(document).ready(function($) {
			...
			$('#ql_form_submit').on('click', function() {
				...
				var nonce = $('#pl_user_quick_login_field').val();
				...
				$.ajax({
					...
					data: {
						action: 'pl_custom_user_login', // The action to be performed on the server
						username: username,
						password: password,
						pl_user_quick_login_field: nonce, // Pass the nonce value
					},
...
add_shortcode( 'pl_quick_login_right_bottom', 'pl_quick_login_right_bottom_func' );


//I added the shortcode to header o run all around the website.
function pl_custom_shortcode_header() {
    echo do_shortcode('[pl_quick_login_right_bottom]');
}

add_action('wp_head', 'pl_custom_shortcode_header');
